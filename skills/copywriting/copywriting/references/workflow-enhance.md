...
1. **Screenshots provided** → Use `alp:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `alp:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/alp:scout ext` (preferred) or `/alp:scout` to find files needing enhancement
