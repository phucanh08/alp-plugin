...
Complete workflow for generating design-aligned visual assets using `alp:ai-multimodal` skill.
...
Use `alp:ai-multimodal` skill's image generation capabilities:
...
Use `alp:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `alp:media-processing` skill for post-generation adjustments
