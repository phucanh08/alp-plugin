...
# alp:predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `alp:debug` for bugs, `alp:plan` for already-decided tasks)
...
|---------------|-------|-----|
| Deepen risk scenarios | `alp:scenario` | Feed Risk Summary rows as feature description |
| Create implementation plan | `alp:plan` | Attach Recommendations as constraints to planner |
| High-risk feature implementation | `alp:cook` | Reference CAUTION/STOP items as acceptance gates |
...
```
/alp:predict "Add WebSocket support for real-time notifications"
/alp:predict "Migrate authentication from JWT to session cookies"
/alp:predict "Add multi-tenancy to the database layer"
/alp:predict "Replace REST API with GraphQL" --files src/api/**/*.ts
```
