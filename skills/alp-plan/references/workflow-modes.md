...
4. **Context reminder:** `/alp:cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/alp:scout` to search codebase
...
7. **Context reminder:** `/alp:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/alp:cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/alp:plan red-team {plan-directory-path}`.
```
/alp:plan red-team {plan-directory-path}
```
...
| `auto` | Run `/alp:plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
/alp:plan validate {plan-directory-path}
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/alp:cook --auto {path}/plan.md` |
| hard | `/alp:cook {path}/plan.md` |
| parallel | `/alp:cook --parallel {path}/plan.md` |
| two | `/alp:cook {path}/plan.md` |
