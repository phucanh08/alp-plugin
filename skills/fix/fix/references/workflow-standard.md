...
2. Activate `alp:debug` skill. Use `debugger` subagent if needed.
3. Activate `alp:sequential-thinking` — form hypotheses through structured reasoning.
...
- Apply `alp:problem-solving` skill if stuck
- Use `alp:sequential-thinking` for complex logic
...
- Activate `alp:project-management` for task sync-back and plan status updates
...
| 2 | `alp:debug`, `alp:sequential-thinking`, `debugger` subagent, parallel `Explore`, (`alp:problem-solving` auto) |
| 3 | `alp:problem-solving` (if stuck), `alp:sequential-thinking` (complex logic) |
| 4 | `tester` subagent, parallel `Bash` verification |
| 5 | `code-reviewer` subagent |
| 6 | `alp:project-management`, `git-manager`, `docs-manager` subagents |
