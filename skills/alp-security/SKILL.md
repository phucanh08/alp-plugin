---
name: alp:security
description: "STRIDE + OWASP-based security audit with optional auto-fix. Scans code for vulnerabilities, categorizes by severity, and can iteratively fix findings using alp:autoresearch pattern."
...
# alp:security — Security Audit

Runs a structured STRIDE + OWASP security audit on a given scope. Produces a severity-ranked findings report. With `--fix`, applies fixes iteratively using the alp:autoresearch guard pattern.
...
|------|-----------|----------|
| Audit only | `/alp:security <scope>` | Scan → categorize → report |
| Audit + Fix | `/alp:security <scope> --fix` | Scan → categorize → fix iteratively |
| Bounded fix | `/alp:security <scope> --fix --iterations N` | Limit fix iterations to N |
...
4. Uses `alp:autoresearch` guard pattern for regression prevention
...
- Run after `alp:predict` when the security persona flags concerns
- Feed Critical/High findings into `alp:autoresearch --fix` for automated remediation
- Use `alp:scenario` with `--focus authorization` for deeper auth flow testing
- Pair with `alp:plan` to schedule Medium/Low findings as sprint tasks
...
/alp:security src/api/**/*.ts

# Audit entire src/ and auto-fix, max 15 iterations
/alp:security src/ --fix --iterations 15
