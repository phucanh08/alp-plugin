...
- **Codebase:** `alp:docs-seeker` skill for package/plugin docs; `alp:repomix` skill for codebase summary
- **Scouting:** `/alp:scout` or `/alp:scout ext` for finding relevant files
- **Frontend:** Chrome browser or `alp:chrome-devtools` skill for visual verification (screenshots, console, network)
- **Skills:** Activate `alp:problem-solving` skill when stuck on complex issues
