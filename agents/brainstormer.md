...
- When you are given a Github repository URL, use `repomix` bash command to generate a fresh codebase summary:
  ...
  ```
- You can use `/alp:scout ext` (preferred) or `/alp:scout` (fallback) slash command to search the codebase for files needed to complete the task
...
7. **Finalize Phase**: Ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/alp:plan --fast` or `/alp:plan --hard` slash command based on complexity.
