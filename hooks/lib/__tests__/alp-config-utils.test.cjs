/**
 * Tests for alp-config-utils edge case handling
 * Run: node .claude/hooks/lib/__tests__/alp-config-utils.test.cjs
 */
...
function normalizeAssertPath(value) {
  if (typeof value !== 'string') return value;
  let normalized = value.replace(/\\/g, '/').replace(/\/+$/g, '');
  return process.platform === 'win32' ? normalized.toLowerCase() : normalized;
}

function assertPathEquals(actual, expected, msg = '') {
  const normalizedActual = normalizeAssertPath(actual);
  const normalizedExpected = normalizeAssertPath(expected);
...
test('getReportsPath returns absolute path when baseDir provided', () => {
  ...
  assertPathEquals(result, '/home/user/project/plans/reports');
});
...
test('getReportsPath uses plan path for session-resolved plans', () => {
  ...
  assertPathEquals(result, '/home/user/project/plans/my-plan/reports');
});
...
test('getGitRoot returns null for non-git directory', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-no-git-' + Date.now());
...
test('path.join concatenates paths (does NOT discard baseDir)', () => {
  ...
  assertPathEquals(result, '/home/user/project/tmp/all-plans');
});

console.log('\n=== Detached HEAD state tests ===\n');

test('getGitBranch returns null or empty in detached HEAD state', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-detached-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
...
test('getGitRoot works in detached HEAD state', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-detached-root-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
    execSync('git config user.email "test@test.com"', { cwd: tempDir });
    ...
    const result = getGitRoot(tempDir);
    assertPathEquals(result, tempDir);
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
});
...
test('getGitRoot returns null for bare repository (no working tree)', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-bare-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
...
test('getGitBranch returns null for bare repository (no HEAD ref)', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-bare-branch-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q --bare', { cwd: tempDir });
...
test('getGitRoot returns innermost repo for nested git repos', () => {
  const outerDir = path.join(os.tmpdir(), 'alp-test-nested-outer-' + Date.now());
...
test('getGitRoot from nested subdir returns correct root', () => {
  const outerDir = path.join(os.tmpdir(), 'alp-test-nested-sub-' + Date.now());
  const innerDir = path.join(outerDir, 'inner');
  ...
  try {
    ...
    assertPathEquals(result, innerDir);
  } finally {
    fs.rmSync(outerDir, { recursive: true, force: true });
  }
});
...
test('getGitRoot resolves through symlink to git repo', () => {
  const realDir = path.join(os.tmpdir(), 'alp-test-real-' + Date.now());
  const linkDir = path.join(os.tmpdir(), 'alp-test-link-' + Date.now());
  ...
  try {
    ...
    if (result !== null) {
      ...
      assertPathEquals(fs.realpathSync(result), fs.realpathSync(realDir));
    }
  } finally {
...
test('getGitRoot with symlinked subdirectory', () => {
  const realDir = path.join(os.tmpdir(), 'alp-test-real-sub-' + Date.now());
  const subDir = path.join(realDir, 'subdir');
  const linkToSub = path.join(os.tmpdir(), 'alp-test-link-sub-' + Date.now());
...
test('getGitRoot works with git worktree', () => {
  const mainDir = path.join(os.tmpdir(), 'alp-test-wt-main-' + Date.now());
  const worktreeDir = path.join(os.tmpdir(), 'alp-test-wt-tree-' + Date.now());
  ...
  try {
    ...
    assertPathEquals(result, worktreeDir);
...
test('getGitRoot works with unicode characters in path', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-日本語-émoji-🔥-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('getGitBranch works with unicode branch name', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-unicode-branch-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
    execSync('git config user.email "test@test.com"', { cwd: tempDir });
...
test('getGitRoot works with special shell characters in path', () => {
  ...
  const tempDir = path.join(os.tmpdir(), "alp-test-special-$var-'quote'-" + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('getGitRoot works on new repo with no commits', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-empty-repo-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('getGitBranch on new repo returns default branch', () => {
  const tempDir = path.join(os.tmpdir(), 'alp-test-new-repo-branch-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('resolvePlanPath returns absolute path as-is (Issue #335)', () => {
  ...
  try {
    ...
    assertPathEquals(result.path, '/project/subfolder/plans/260111-feature');
  } finally {
