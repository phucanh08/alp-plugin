/**
 * Shared utilities for AnhlpKit hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.alp.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.alp.json');
...
function getSessionTempPath(sessionId) {
  return path.join(os.tmpdir(), `alp-session-${sessionId}.json`);
}
 ...
 * @param {Object} config - AnhlpKit config
      ...
      case 'session': {
        ...
        if (state?.activePlan) {
          ...
          if (!path.isAbsolute(resolvedPath) && state.sessionOrigin) {
            ...
            // Preserve POSIX-style sessionOrigin strings in tests/config even on Windows.
            if (state.sessionOrigin.startsWith('/') && !/^[A-Za-z]:/.test(state.sessionOrigin)) {
              resolvedPath = `${state.sessionOrigin.replace(/[\\/]+$/, '')}/${resolvedPath.replace(/\\/g, '/')}`;
            } else {
...
function sanitizePath(pathValue, projectRoot) {
  ...
  // Normalize projectRoot first so POSIX-like test paths (e.g. /project)
  // compare correctly on Windows too.
  ...
  if (!compareResolved.startsWith(compareRoot + path.sep) && compareResolved !== compareRoot) {
 ...
 *   2. Global config (~/.claude/.alp.json) - user preferences
 *   3. Local config (./.claude/.alp.json) - project-specific overrides
 *
...
function getReportsPath(planPath, resolvedBy, planConfig, pathsConfig, baseDir = null) {
  ...
  if (baseDir) {
    ...
    // Preserve POSIX-style baseDir strings in tests/config even on Windows.
    if (baseDir.startsWith('/') && !/^[A-Za-z]:/.test(baseDir)) {
      return `${baseDir.replace(/[\\/]+$/, '')}/${normalizedReportPath}`;
    }
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.ALP_DEBUG) {
      console.error(`[alp-config] Warning: ${validation.error}`);
    }
  }
...
function getGitRoot(cwd = null) {
  const root = execSafe('git rev-parse --show-toplevel', { cwd: cwd || undefined });
